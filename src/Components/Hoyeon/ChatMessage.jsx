...
const ChatMessage = ({ warnNkick, roomNumber, user, chatbanstatus, banWordList, setBanWordList, chatType, friend, roomData, setReloadState }) => {
  ...
  const openSocket = () => {
    ...
    let socket = null;
    if(textChatNo !== 0){
      socket = new WebSocket(`ws://192.168.0.140:9090/chat?userCode=${senderCode}&textChatNo=${textChatNo}`);
    }else {
      socket = new WebSocket(`ws://192.168.0.140:9090/chat?userCode=${senderCode}&textChatNo=0`)
    }
    socket.onopen = () => {
    ...
    socket.onmessage = (msg) => {
      ...
      try {
        ...
        console.log("dadwadadawdwadwadwadwadawda", data.chatImg);
  ...
  <Container>
    ...
    <VideoCall user={user} friend={friend}/>
     {
      ...
      <ChattingArea id='bodyArea' className='backgCon'>
    ...
    <FooterArea className='backgCon'>
        <InputContainer>
          ...
          {/* 오른쪽 아이콘들 */}
          <IconButton>
            <i className="bi bi-headset"  onClick={toggleVideoCall}></i> {/* 헤드셋 아이콘 */}
          </IconButton>
